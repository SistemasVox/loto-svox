...
export default function ResultadosPage() {
  ...
  // Loading visual com delay mínimo de 1.5s
  ...
  }, [loading, hasMore, fetchResultados, page]);

  // ========== LOADING VISUAL GLOBAL ==========
  ...
  return (
    ...
    >
      {/* Fundo da logo centralizada e responsiva */}
      <div
        aria-hidden="true"
        style={{
          position: "fixed",
          ...
          backgroundSize: "cover", // Aqui é o segredo!
          opacity: 0.13,           // Ajuste conforme o gosto!
      ...
      {!erro && resultados.length === 0 && !loading && (
        <p className="text-center text-gray-400 relative z-10">Nenhum resultado encontrado.</p>
      )}
          ...
          >
            <div className="w-full text-xs text-gray-400 mb-1 text-center">
              ...
              <span className="text-gray-300 font-semibold">
                ({new Date(r.data_concurso).toLocaleDateString()})
              </span>
            </div>
      ...
      {/* LOADING AO PAGINAR MAIS RESULTADOS */}
      ...
      {!hasMore && !loading && (
        <p className="text-center text-gray-400 mt-6 relative z-10">Fim dos resultados :)</p>
      )}

      {/* CSS extra para o efeito bola */}
      <style>{`
        .dezena-bola {
          display: flex;
          align-items: center;
